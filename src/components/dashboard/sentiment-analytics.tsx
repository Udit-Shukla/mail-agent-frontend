...
export function SentimentAnalytics() {
  ...
  // Debug logging
  useEffect(() => {
    console.log('🔍 SentimentAnalytics Debug:', {
    ...
    if (analyticsData) {
      console.log('📊 SentimentAnalytics - Full analytics data:', analyticsData);
      console.log('📊 SentimentAnalytics - Sentiment data:', analyticsData.sentiment);
    }
    ...
    <Card className="bg-card/50 backdrop-blur-sm border-border/50 shadow-sm">
      <CardHeader className="pb-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <CardTitle className="text-lg font-semibold">Email Sentiment</CardTitle>
            <p className="text-sm text-muted-foreground">Tone analysis</p>
          </div>
        </div>
      </CardHeader>
      <CardContent className="space-y-3">
        {analyticsData.sentiment.map((item) => (
          <button
            key={item.name}
